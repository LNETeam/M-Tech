...
function bootCheck()
	if !bootFrmk.checkBootAuth() then
		error("0x001C")
	end
end
...
if bootOkay then
	...
	if !bootFrmk.verifyVersionIdentity(manifest_file_contents,integratedBootVersion) then
		kernel.throwMismatchLogEvent()
	end
	if !bootFrmk.verifyInstallID(manifest_file_contents) then
		kernel.throwIllegalInstallEvent()
	end
	shell.setDir("")
	if !boot_instructions.checkAndDecompress then
		kernel.throwDecompressEvent()
	end
	shell.run("/sys/boot/main.interface")
else
